...
from models import CreateBookingRequest, PayFineRequest, OfficerExitRequest
...
async def create_booking(data: CreateBookingRequest):
    ...
    booking = {
        ...
        "payment_status": "pending" if (data.payment_method or "").lower() == "cash" else "paid",
...
@router.post("/{booking_id}/officer-exit")
async def officer_permit_exit(booking_id: str, data: OfficerExitRequest):
    """Security officer permits the vehicle to exit at the gate.
    For cash-on-delivery bookings, the officer must confirm cash was collected first."""
    db = get_db()
    booking = await db.bookings.find_one({"booking_id": booking_id.upper()})
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found")
    ...
    if booking.get("status") == "completed":
        return {
            ...
            "message": "This ticket was already permitted to exit"
        }

    payment_method = (booking.get("payment_method") or "card").lower()
    update_fields = {"status": "completed", "exited_at": datetime.utcnow()}

    if payment_method == "cash" and booking.get("payment_status") != "paid":
        if not data.payment_collected:
            raise HTTPException(status_code=400, detail="Cash payment must be collected before permitting exit")
        update_fields["payment_status"] = "paid"
        update_fields["cash_collected_at"] = datetime.utcnow()

    await db.bookings.update_one({"booking_id": booking_id.upper()}, {"$set": update_fields})
    ...
    await db.lots.update_one({"lot_id": booking["lot_id"]}, {"$inc": {"available_slots": 1}})
